...
class SummaryStats():
    ...
    """
    def __init__(self,mean_val:float,std:float, n:int):
        """
        :param mean_val: mean
        :param std: standard deviation
        :param n: number of observations used to determine above statistics
        """
...
class SimilarityCalculatorNavigator:
    ...
    def get_stats(self,similarities:list)->SummaryStats:
        """
        Calculate summary statistics for list of similarities
        :param similarities:
        :return: object with mean, stdev and n of measurments
        """
        if self.rm_outliers:
            similarities=self.remove_outliers(similarities)
        m=mean(similarities)
        std=stdev(similarities)
        n=len(similarities)
        return SummaryStats(m,std,n)
...
class RandomSimilarityCalculatorNavigator(SimilarityCalculatorNavigator):
    ...
    def similarities(self,n_pairs:int,adjust_n:bool=False )->SummaryStats:
        """
        ...
        :return: object containing mean, standard deviation and n of similarities
        """
        ...
        return self.get_stats(similarities_list)
...
class GeneSetSimilarityCalculatorNavigator(SimilarityCalculatorNavigator):
    ...
    def similarities(self,geneIDs:list ,max_n_similarities:int=None)->SummaryStats:
        """
...
#Solwed elsewhere: TODO if number of genes is very small in some groups the distances may not be representative
    def get(self,number:int,adjust_n:bool=False):
        """
