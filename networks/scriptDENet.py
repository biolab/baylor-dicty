...
for sampleStrain,reps in samples.items():
    for replicate in reps:
        ...
        genesWT, genesWTN = f.genesByStrain(genesNotNull, table, data['Strain'], sampleStrain + '_r' + str(repN),genesFromRow)
        f.buildGraph(graph, sampleStrain, replicate, rScoresS, threshold, genesWTN,threshRorP=True, pScores='')
        f.saveGraph(graph, dataPathSaved + conditions.replace('.', '_') + thresholdRStr + "_"+sampleStrain+rep,False)
...
thresholdRStr = 'Tr' + str(newTr).replace('.', '_')
sampleStrain='AX4'
fileName = (sampleStrain + '_' + 'kN' + str(knnNeighbours) + 'T' + str(thresholdKNND) + thresholdRStr).replace('.','_')+'_Trimmed.gml'
...
conditionsMerg=('kN' + str(knnNeighbours) + 'T' + str(thresholdKNND) + thresholdRStr).replace('.','_')+'_MergedStrains'
...
f.saveGenesForNet(graph297,genes,table,dataPathSaved+'kN300T0_3Tr0_97_MergedStrainsminStrain2.tsv',genesFromRow,False)
...
f.saveGenesForNet(pruned,genes,table,dataPathSaved+conditions+thresholdRStr+prunStr+'subsetForNetworkNoIsolates.tsv',genesFromRow)
...
f.saveGenesForNet(graph,genes,table,dataPathSaved+conditions+thresholdRStr+'subsetForNetworkNoIsolates.tsv',genesFromRow)

#Make graph for min(abs(r))=0.8 and present in all strains
#Name r score lists
for sampleStrain,reps in samples.items():
    for replicate in reps:
        conditionR=(sampleStrain+'_rep'+str(replicate)+'kN'+str(knnNeighbours)+'T'+ str(thresholdKNND)).replace('.','_')
        rScores=f.loadRScores(dataPathSaved+conditionR)
        genesWT, genesWTN = f.genesByStrain(genesNotNull, table, data['Strain'], sampleStrain + '_r' + str(replicate), genesFromRow)
...
conditionMerged=('kN'+str(knnNeighbours)+'T'+ str(thresholdKNND)+'Tr'+str(threshold)).replace('.','_')+'_Present'+str(minStrains)
