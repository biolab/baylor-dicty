...
#Genes table on which the calculations will be donem, genes in rows
#scaleByAxis - scale axis to have stdev 1 and mean 0
#filePrefix - file prefix and path to save the results
#Returns distances and neighbours data for original data and iversed profiles, respectively and number of used genes
def genesKNN(kN,genes,scaleByAxis,filePrefix):
    ...
    neigh = resultKNN[0]
    # Inverse result
    inverse = pd.DataFrame(scaled) * -1
    resultKNNInv = index.query(inverse.values.tolist(), k=kN)
...
def saveTopCorrelPairsByP(filePrefix,pScores,rScores,gen,top):
    ...
    for idx in topIdxs(top,pScores):
        j,i=idx
        f.write(gen.iloc[i].name+' '+gen.iloc[j].name+ ' p '+ str(pScores[idx])+ ' r '+str(rScores[idx])+'\n')
...
def mergeRScoresRep(table,genesNotNull,collumnStrains,repDict,fromRow,path,knnNumber,thresholdKnn,thresholdRScores):
...
def tsneScatter(associations,perplexity,weigthMin=0,random=0,openTsne=True):
    ...
    if not openTsne:
        ...
        for j in range(n):
            narrayD[j,:]=matrixD[j,:]
        affinities_train = PerplexityBasedNN(narrayD,perplexity=perplexity, method='predefined', metric="predefined",random_state=random)
