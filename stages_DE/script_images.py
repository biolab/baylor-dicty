...
measurement_images=[conditions_images[conditions_images['Image']==image]['Measurment'].values[0]
...
for query,data in cosine_sims.iterrows():
    ...
    measurements_best=data.index[-4:]
    ...
    statistic=statistic.replace(np.nan,0)
...
plt.hist(gene_vars_similar.median(axis=1),bins=100,alpha=0.4,label='median')
plt.hist(gene_vars_similar.mean(axis=1),bins=100,alpha=0.4,label='mean')
plt.legend()
# Remove genes with median 0 deviance
var_medians=gene_vars_similar.median(axis=1)
remove_genes=var_medians[var_medians==0].index
gene_vars_similar=gene_vars_similar.drop(remove_genes)
...
gene_vars_all = genes.loc[gene_vars_similar.index,:].std(axis=1)/genes.loc[gene_vars_similar.index,:].mean(axis=1)
gene_vars_diff=(gene_vars_all.T-gene_vars_similar.T).T
#TODO try also all/similar
# Some genes may be naturally more variable
